...
function figureMethod(spec) {
  ...
  return function () {
    ...
    for (var i = 0, l = urlParams.length; i < l; ++i) {
      ...
      if (!arg) {
        var err = new Error('Figure: Argument "' + urlParams[i] + '" required, but got: "' + arg + ' (on API request to ' + requestMethod + ' ' + commandPath + ')');
        deferred.reject(err);
    ...
    if (args.length) {
      var err = new Error('Figure: Unknown arguments (' + args + ').');
      deferred.reject(err);
      return deferred.promise;
    }
